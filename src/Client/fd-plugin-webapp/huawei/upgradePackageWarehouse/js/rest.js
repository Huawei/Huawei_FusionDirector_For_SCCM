  ...
  var upgradePackageManage = {
      ...
      getBaselineList: function (param, callback) {
          ...
          endpoint += "$skip=" + (param.pageNo - 1) * param.pageSize + "&$top=" + param.pageSize+"&$orderby=BaselineName asc";
          if(param.filter){
            endpoint += "&$filter=BaselineName has '" + param.filter+"'"
          }
          var data = {
              "ips": [param.ip],
              "httpMethod": 'GET',
      ...
      getPackageList: function (param, callback) {
            var endpoint = '/rich/UpgradeService/Repository?';
            endpoint += "$skip=" + (param.pageNo - 1) * param.pageSize + "&$top=" + param.pageSize+"&$orderby= PackageName asc";
            if(param.filter){
                endpoint += "&$filter= "+param.type+" has '"+param.filter+"'";
            }
            var data = {
                "ips": [param.ip],
                "httpMethod": 'GET',
       ...
       * @param {*} callback 
       */
      uploadPackageFile: function (param, callback) {
        ...
        var param = {
            "ips": [param.ip],
            "httpMethod": "POST",
            "endpoint": '/rich/UpgradeService/Actions/Package.Import',
            "data": param.data
        }
        var ayncResult = NetBound.execute("uploadPackage", param);
        ExecuteAnynsMethodOnlyHandlerPromise(ayncResult, (resultStr) => {
            var resultJson = JSON.parse(resultStr);
               var ret = {
                   ...
                   ip: resultJson.ips ? resultJson.ips[0] : null
               }
               callback(ret);
        });
    },
    cancelPackageUpload: function (param, callback) {
        if (window.NetBound == undefined || window.NetBound == null || !window.NetBound) {
            //判断cefBrowser是否已注册JsObject
            alert('window.NetBound does not exist.');
            console.log('window.NetBound does not exist.');
        ...
        var ayncResult = NetBound.execute("cancelPackageUpload", param);
        ExecuteAnynsMethodOnlyHandlerPromise(ayncResult, (resultStr) => {
            var resultJson = JSON.parse(resultStr);
               var ret = {
                   code: resultJson.code,
    ...
    /**
       ...
       * @param {*} callback 
       */
      uploadAscFile: function (param, callback) {
        if (window.NetBound == undefined || window.NetBound == null || !window.NetBound) {
            //判断cefBrowser是否已注册JsObject
            alert('window.NetBound does not exist.');
            console.log('window.NetBound does not exist.');
        ...
        var ayncResult = NetBound.execute("uploadSignature", param);
        ExecuteAnynsMethodOnlyHandlerPromise(ayncResult, (resultStr) => {
            var resultJson = JSON.parse(resultStr);
               var ret = {
                   code: resultJson.code,
    ...
    cancelAscUpload: function (param, callback) {
        if (window.NetBound == undefined || window.NetBound == null || !window.NetBound) {
            //判断cefBrowser是否已注册JsObject
            alert('window.NetBound does not exist.');
            console.log('window.NetBound does not exist.');
        ...
        var ayncResult = NetBound.execute("cancelSignatureUpload", param);
        ExecuteAnynsMethodOnlyHandlerPromise(ayncResult, (resultStr) => {
            var resultJson = JSON.parse(resultStr);
               var ret = {
                   code: resultJson.code,
