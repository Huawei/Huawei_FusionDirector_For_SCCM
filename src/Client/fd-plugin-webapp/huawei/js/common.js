 ...
 * @param {获取} data 
 */
function getDate(str) { //解决new Date()IE10不支持参数
...
function ajax(data, callback) {
    ...
    ExecuteAnynsMethodOnlyHandlerPromise(ayncResult, (resultStr) => {
        ...
        if (typeof callback === "function") {
            ...
            if (ret.code == '0') {
                console.log(ret.msg.indexOf("-") != -1)
                if (ret.msg.indexOf("-") != -1) {
                    var data = parseTime(new Date(getDate(ret.msg).getTime() + 5 * 60 * 1000));
                    sessionStorage.setItem('serverDate1', parseTime(new Date(getDate(ret.msg).getTime())));
                    sessionStorage.setItem('serverDate', data);
                } else {
                    var serverDate1 = parseTime(new Date(new Date(ret.msg).getTime()));
                    console.log(serverDate1)
                    sessionStorage.setItem('serverDate1', serverDate1);
                    var data = parseTime(new Date(new Date(ret.msg).getTime() + 5 * 60 * 1000));
                    sessionStorage.setItem('serverDate', data);
                }
            }
...
function isAlert(ret, item) {
    ...
    if (!sessionStorage.getItem(item)) {
        if (ret.data.length == 0 || ret.data[0].data === undefined ||
