    ...
    public class EncryptUtil
    {
...
#else
        static string rootPath = Environment.GetEnvironmentVariable("PUBLIC");
#endif

        //密钥组件分散保存，当密钥组件存储于文件中时，不能所有的组件文件放在同一目录下存储；
        //因此加多一层目录.
        static string rootFile = Path.Combine(rootPath, "Huawei\\SCCM Plugin For Fusion Director\\base.kn");
        static string mainFile = Path.Combine(rootPath, "Huawei\\SCCM Plugin For Fusion Director\\main\\main.kn");
        /// <summary>
        /// 获得根密钥
        /// </summary>
        /// <returns>根密钥</returns>
        ...
        )
        {
           ...
           */
            ...
            OpenSSLApi.PKCS5_PBKDF2_HMAC(passPhrase, 16, saltValueBytes, 16, pdIterations, OpenSSLApi.EVP_sha256(), 32, keyBytes);
        ...
        /// 加密
        /// </summary>
        /// <param name="pdStr">需要加密的密码</param>
        /// <returns>加密后的密码</returns>
        public static string EncryptPwd(string pdStr)
        {
            return EncryptWithKey(GetMainKeyFromPath(), pdStr);
        }
        /// <summary>
        /// 解密
        /// </summary>
        /// <param name="pdStr">需要解密的密码</param>
        /// <returns>解密后的密码</returns>
        public static string DecryptPwd(string pdStr)
        {
            return DecryptWithKey(GetMainKeyFromPath(), pdStr);
        }

        #region 兼容旧的密钥
        static string versionFile = Path.Combine(rootPath, "Huawei\\SCCM Plugin For Fusion Director\\vrn.kn");
