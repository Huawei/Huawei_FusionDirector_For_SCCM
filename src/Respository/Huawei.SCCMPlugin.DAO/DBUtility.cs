        ...
        private static string getAndInitDatabase()
        {
            ...
            try
            {
                using (var mutex = new System.Threading.Mutex(false, "huawei.sccmplugin.db"))
                {
                    ...
                    {
                        var localPath = System.Environment.GetEnvironmentVariable("userprofile");//C:\Users\Public\Huawei\SCCM Plugin For Fusion Director

                        var allUserPath = System.Environment.GetEnvironmentVariable("PUBLIC");

                        userDBPath = Path.Combine(localPath , "Huawei","SCCM Plugin For Fusion Director","DB", "FusionDirectorDB.sqlite");
                        string allDBPath = Path.Combine(allUserPath , "Huawei","SCCM Plugin For Fusion Director","DB", "FusionDirectorDB.sqlite");
